...
#define i2c_write(a, b, c, d)   twi_write(a, b, d, c)       // twi_write()是通过字节写，a:i2c从机地址，b:寄存器地址，c:字节数，d:数据
...
/* Hardware registers needed by driver. 存储寄存器地址*/
struct gyro_reg_s {
...
/* Cached chip configuration data.  存储配置数据
...
//  定义并初始化 gyro_state_s st
static struct gyro_state_s st={
  &reg,
  &hw,
  {0},      //这里是c结构体的一种初始化方法，改子结构体chip_cfg_s chip_cfg全初始化为0
  &test
};
...
/** 如果dmp已开启，enable=1，则开启DMP interrupt；如果dmp已关闭，enable=1，则开启data ready interrupt；
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            tmp = BIT_DMP_INT_EN;    //DMP int是在bit 1。这一点register map上没有说明。
        else
            tmp = 0x00;
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (mpu_set_lpf(42))
        return -1;
    if (mpu_set_sample_rate(50))   //mpu_sample_rate是通过设置SMPLRT_DIV寄存器，详情参考register map
        return -1;
    if (mpu_configure_fifo(0))  //mpu_fifo在register map上没有说明，从这里得出是使用的通用FIFO（mpu6050就一个FIFO）。
        return -1;
    ...
    mpu_set_sensors(0);     //关闭所有传感器，并进入省电模式（low power mode）
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    if (st.chip_cfg.dmp_on) {
        data = BIT_FIFO_RST | BIT_DMP_RST;      //从这里看出USER_CTRL bit3是dmp reset位
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
            return -1;
       
        data = BIT_DMP_EN | BIT_FIFO_EN;    //从这里可以看出SER_CTRL bit7是dmp en位
        ...
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
        data = 0;
        if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, &data))      //从这里可以看出，如果dmp打开了，那么不把任何传感器的值传入FIFO中
            return -1;
    } else {
        data = BIT_FIFO_RST;
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
...
/** 注：当没有一个传感器（sensors：mpu上的陀螺仪，加速度传感器等）的值要传输给FIFO时，则dmp的值会传给FIFO
...
int mpu_configure_fifo(unsigned char sensors)
{
    ...
    if (st.chip_cfg.dmp_on)     //如果dmp已开启，则直接返回
        return 0;
    else {
        if (!(st.chip_cfg.sensors))     //如果没有一个传感器开启，则错误返回
            return -1;
        prev = st.chip_cfg.fifo_enable;   //st.chip_cfg.fifo_enable用来记录当前的FIFO是否开启
        st.chip_cfg.fifo_enable = sensors & st.chip_cfg.sensors;    //st.chip_cfg.sensors是已开启的传感器
        if (st.chip_cfg.fifo_enable != sensors)
            /* 如果不相等，说明有些传感器还未开启，所有错误返回
        ...
        if (sensors || st.chip_cfg.lp_accel_mode)
            set_int_enable(1);      //MPU6050.c 270行调用时，st.chip_cfg.dmp_on还是0，所以开启的是data ready interrupt
        else
            set_int_enable(0);       
        if (sensors) {
            if (mpu_reset_fifo()) {
...
/** 如果sensors bit为0，则对应的传感器进入standby mode模式，即关闭状态
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_GYRO)
        ...
        data = 0;    //使用 X axis gyroscope作为时钟源，并且不使用 low power sleep mode
    else
        data = BIT_SLEEP;   //low power sleep mode
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
    st.chip_cfg.clk_src = data & ~BIT_SLEEP;    //保存时钟源设置
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_2, 1, &data)) {     //相应的传感器进入 standby mode
        st.chip_cfg.sensors = 0;
        return -1;
    }
    ...
    st.chip_cfg.sensors = sensors;      //保存传感器状态
    st.chip_cfg.lp_accel_mode = 0;      //我们关闭了 low power mode,所以设为0
    delay_ms(50);
    return 0;
}
